...
export default function AuthModal({ isOpen, initialMode = 'login', onClose }) {
  const [mode, setMode] = useState(initialMode); // 'login' | 'register' | 'google-profile'
  ...
  const handleGoogleAuth = () => {
    // Simulated Google OAuth response providing partial identity
    const mockGoogleIdentity = {
      name: 'Indian Trader',
      email: 'trader@propfirm.co',
    };
    setGoogleUser(mockGoogleIdentity);
    setFullName(mockGoogleIdentity.name);
    setEmail(mockGoogleIdentity.email);
    // Since phone number is mandatory per rules, prompt profile completion
  ...
  const handleRegisterSubmit = (e) => {
    ...
    setTimeout(() => {
      window.open(TRADING_APP_URL, '_blank');
      onClose();
    }, 1500);
  };
  ...
  const handleGoogleProfileSubmit = (e) => {
    ...
    if (!validateEmail(email)) {
      setError('Valid Email ID is mandatory.');
      return;
    }
    ...
    setSuccessMsg('Google Account verified! Redirecting to trading terminal...');
    setTimeout(() => {
      window.open(TRADING_APP_URL, '_blank');
      onClose();
    }, 1500);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.88rem', marginTop: '0.25rem' }}>
            {mode === 'login'
              ...
              : 'Name, valid Email ID, and Phone Number are mandatory for verified access.'}
          </p>
        </div>
        ...
        {/* 3. GOOGLE PROFILE COMPLETION (Mandatory Phone & Verification) */}
        {mode === 'google-profile' && (
          <form onSubmit={handleGoogleProfileSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
            <div style={{ background: 'rgba(255,255,255,0.03)', padding: '1rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>GOOGLE ACCOUNT DETECTED</span>
              <div style={{ fontWeight: 700, color: '#ffffff', marginTop: '0.2rem' }}>{googleUser?.name}</div>
              <div style={{ fontSize: '0.82rem', color: 'var(--accent-cyan)' }}>{googleUser?.email}</div>
            </div>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.35rem' }}>
                Valid Phone Number (Mandatory 10-Digit Mobile) *
              </label>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ padding: '0.75rem', background: 'rgba(255,255,255,0.06)', borderRadius: 'var(--radius-md)', color: '#fff', fontFamily: 'var(--font-mono)' }}>
                  +91
                </div>
                <input
                  type="tel"
