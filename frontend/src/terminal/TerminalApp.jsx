...
function TerminalApp({ onExit }) {
  const { accessToken, logout } = useAuth();

  // `user` here is this component's own bootstrap-readiness flag (it's set
  // from getMe() once accounts/instruments have also loaded below) -- it is
  // deliberately separate from AuthContext's `user`, which can already be
  // populated before this component's bootstrap() finishes.
  ...
  const [marginPreviewLoading, setMarginPreviewLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    // non-empty string before it will open a connection. Using the real
    // access token here anyway (instead of a placeholder) so the WS
    // connection carries real credentials the day that endpoint starts
    // checking them.
    wsManager.setToken(accessToken || 'local');
    wsManager.connect();
  ...
  }, [user, accessToken, selectedAccountId, selectedSymbol, futures, callInstrument, putInstrument, optionChain]);

  // Required-vs-available margin for the order ticket currently open, so a
  // trader can see whether they can actually afford this order *before*
  // hitting submit -- computed by the backend (RiskEngine.compute_margin_requirement)
  // so it always matches what submit would actually enforce, not a separate
  // client-side approximation that could drift from the real gate.
  useEffect(() => {
    if (!orderTicket || !selectedAccountId) {
      setMarginPreview(null);
      return;
    }
    const quantity = Math.max(1, Number(orderLots || 1)) * Number(orderTicket.lot_size || 1);
    let cancelled = false;
    setMarginPreviewLoading(true);
    const timer = setTimeout(() => {
      getMarginPreview(selectedAccountId, {
        instrumentId: orderTicket.id,
        side: orderSide,
        quantity,
      })
        .then((preview) => {
          if (!cancelled) setMarginPreview(preview);
        })
        .catch((e) => {
          console.error('Margin preview failed', e);
          if (!cancelled) setMarginPreview(null);
        })
        .finally(() => {
          if (!cancelled) setMarginPreviewLoading(false);
        });
    }, 250); // debounce lot-count keystrokes
    return () => {
      cancelled = true;
      clearTimeout(timer);
    };
  }, [orderTicket, orderSide, orderLots, selectedAccountId]);
      ...
      <header className="top-bar">
        ...
        <div className="ticker-strip">
          {topTickers.map((symbol) => {
            ...
            let ch = q?.change !== undefined && q?.change !== null ? Number(q.change) : null;
            let chPct = q?.change_pct !== undefined && q?.change_pct !== null ? Number(q.change_pct) : null;
            if ((ch === null || ch === 0) && q?.ltp && q?.prev_close && Number(q.prev_close) !== 0) {
              ch = Number(q.ltp) - Number(q.prev_close);
              chPct = (ch / Number(q.prev_close)) * 100;
            }
            const displayCh = ch !== null ? ch : 0;
            return (
              <div key={symbol} className="ticker-item" title={q ? `bid ${Number(q.bid).toFixed(2)} / ask ${Number(q.ask).toFixed(2)} · vol ${q.volume} · ${q.source}` : ''}>
                ...
                <span className={displayCh >= 0 ? 'up-text' : 'down-text'}>
                  {q ? `${displayCh >= 0 ? '+' : ''}${displayCh.toFixed(2)} (${formatPercentage(chPct)})` : '--'}
                </span>
              </div>
            );
          })}
      ...
      {orderTicket && (
        <div className="modal-backdrop" onClick={() => setOrderTicket(null)}>
          <div className="order-modal" onClick={(event) => event.stopPropagation()}>
            ...
            {(() => {
              const calculatedLots = Math.max(1, Number(orderLots || 1));
              const calculatedQty = calculatedLots * Number(orderTicket.lot_size || 1);
              const ticketLtp = quotes[orderTicket.trading_symbol]?.ltp ?? orderTicket.ltp ?? 0;
              const estRequiredFunds = orderSide === 'BUY'
                ? calculatedQty * Number(ticketLtp)
                : (Number(orderTicket.strike_price || ticketLtp || 25000) * Number(orderTicket.lot_size || 1) * 0.15) * calculatedLots;
              const availableEquity = summary ? Number(summary.equity || 0) : Number(activeAccount?.starting_balance || 50000);
              const estAvailableMargin = Math.max(0, availableEquity - (summary ? Number(summary.funds_used || 0) : 0));

              const required = marginPreview ? Number(marginPreview.required_amount) : estRequiredFunds;
              const available = marginPreview ? Number(marginPreview.available_amount) : estAvailableMargin;
              const sufficient = marginPreview ? marginPreview.sufficient : (available >= required);

              return (
                <div className="margin-card">
                  <div className="margin-row header-row">
                    <span className="margin-title">Funds & Margin Verification</span>
                    <span className={`status-pill ${sufficient ? 'sufficient' : 'insufficient'}`}>
                      {sufficient ? '✓ Sufficient Margin' : '⚠ Insufficient Funds'}
                    </span>
                  </div>
                  <div className="margin-grid">
                    <div className="margin-box">
                      <span className="margin-label">{orderSide === 'BUY' ? 'Funds Required' : 'Required Margin'}</span>
                      <span className="margin-value required-val">₹{required.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                    <div className="margin-box">
                      <span className="margin-label">Available Margin</span>
                      <span className="margin-value available-val">₹{available.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                  <div className="margin-meta">
                    <span>Order Qty: <strong>{calculatedLots} Lot{calculatedLots > 1 ? 's' : ''} ({calculatedQty} Qty)</strong></span>
                    {ticketLtp > 0 && <span>LTP: <strong>₹{Number(ticketLtp).toFixed(2)}</strong></span>}
                  </div>
                  {!sufficient && (
                    <div className="margin-alert">
                      Shortfall: ₹{(required - available).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}. Reduce lot size or select an account with higher capital.
                    </div>
                  )}
                </div>
              );
            ...
            <button
              ...
              disabled={orderSubmitting || (marginPreview ? !marginPreview.sufficient : (summary ? Number(summary.equity || 0) < (Math.max(1, Number(orderLots || 1)) * Number(orderTicket.lot_size || 1) * Number(quotes[orderTicket.trading_symbol]?.ltp ?? orderTicket.ltp ?? 0)) : false))}
